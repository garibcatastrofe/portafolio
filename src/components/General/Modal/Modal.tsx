...
export function Modal() {
  const { isActivated, setModal, modalTitle, modalBody } =
    useModal();
  ...
  return (
    ...
    >
      ...
      <motion.div
        className={`z-[70] bg-stone-800 p-4 rounded-lg shadow-lg lg:w-[600px] md:w-[500px] w-[calc(100%-2rem)]`}
      ...
      >
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="w-fit h-fit">{modalTitle}</div>
          <HiOutlineX
            onClick={hacerModalFalso}
            className="text-3xl transition duration-200 cursor-pointer text-stone-50 hover:text-escuela"
          />
        </div>
